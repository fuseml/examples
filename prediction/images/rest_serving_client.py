...
parser.add_argument('--classes', default=None, help='JSON file with labels.', dest='classes')
...
while iteration <= iterations:
    for x in range(0, imgs.shape[0] - batch_size + 1, batch_size):
        ...
        result = session.post("{}/v1/models/{}{}:predict".format(args['rest_url'], args['model_name'], version), data=data_json, cert=certs, verify=verify_server)
            ...
            if args.get('labels_numpy_path') is not None:
                ...
                if ma == lb[i]:
                    ...
                    mark_message = "; Incorrect match. Should be {} {}".format(
                        classes.get(str(lb[i]), lb[i]), lb[i])
            print("\t", i, classes.get(str(ma), ma), ma, mark_message)
        # Comment out this section for non imagenet datasets
