...
def images_to_npy(image_dir,label_file,classes,input_height,input_width,input_chans,resize,normalize,one_hot,compress,output_file,output_label_file):
  ...
  save=np.save
  if (compress):
    save=np.save_compressed
  
  if (output_label_file):
    save(output_file, x)
    save(output_label_file, y)
    print(' Data saved to',output_file)
    print(' Labels saved to',output_label_file)
  else:
    save(output_file, x=x, y=y)
...
def run_main():
  ...
  ap.add_argument('-ol','--output_label_file',   type=str,   default='', help='Full path of output labels file.')
  args = ap.parse_args()
  ...
  print('--output_label_file   : ',args.output_label_file  )
  print(DIVIDER)
  

  images_to_npy(args.image_dir, args.label_file, args.classes, args.input_height, \
  args.input_width, args.input_chans, args.resize, args.normalize, args.one_hot, \
  args.compress, args.output_file, args.output_label_file)
